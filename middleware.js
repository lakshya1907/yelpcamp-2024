...
// CSRF protection: synchronizer token pattern using the session.
// A token is generated per session (see app.js) and must be echoed back
// in a hidden form field named `_csrf` on every state-changing request.
module.exports.verifyCsrf = (req, res, next) => {
    const tokenFromForm = req.body._csrf;
    if (!tokenFromForm || tokenFromForm !== req.session.csrfToken) {
        throw new ExpressError('Invalid or missing CSRF token', 403);
    }
    next();
}
